...
<ui-drag-area class="container">
    <section class="prefab" hidden missing>
        <div class="left">
            ...
            <ui-icon class="edit-prefab" value="exit" tooltip="i18n:hierarchy.menu.edit_prefab"
            ></ui-icon>
        </div>
...
const Elements = {
    ...
    prefab: {
        ready() {
            ...
            panel.$.prefab.addEventListener('confirm', (event) => {
                ...
                panel.dumps.forEach((dump) => {
                    const prefab = dump.__prefab__;
                    // prefab.uuid 资源的
                    // dump.uuid.value 节点的
                    switch (role) {
                        case 'unlink': {
                            Editor.Message.send('hierarchy', 'unlink-prefab', dump.uuid.value);
                            break;
                        }
                        case 'local': {
                        ...
                        case 'reset': {
                            Editor.Message.send('scene', 'restore-prefab', prefab.rootUuid, prefab.uuid);
                            break;
                        }
                        case 'save': {
                            Editor.Message.send('scene', 'apply-prefab', prefab.rootUuid);
                            break;
                        }
                    }
                });
    ...
    scene: {
        ...
        update() {
            ...
            $skyProps.forEach(($prop) => {
                if ($prop.dump.name === 'applyDiffuseMap' ||
                    $prop.dump.name === 'envmap') {
    ...
    missingComponent: {
        ...
        update() {
            ...
            for (let i = 0; i < panel.dump.removedComponents.length; i++) {
                ...
                <ui-icon value="reset" index="${i}"></ui-icon>
                <ui-icon value="save-o" index="${i}"></ui-icon>
                `;
            }
    ...
    layer: {
        ...
        async update() {
            ...
            // const userLayers = await Editor.Profile.getProject('project', 'layer');
            // if (Array.isArray(userLayers)) {
            //     enumList.unshift(...userLayers);
            //     enumList.sort((a: any, b: any) => {
            ...
            menu: [
                {
                    ...
                    click() {
                        Editor.Message.send('scene', 'snapshot');
                        // const comp = dump.__comps__[index];
        ...
        Editor.Menu.popup({
            menu: [
                ...
                { type: 'separator' },
                {
                    ...
                    async click() {
                        Editor.Message.send('scene', 'snapshot');
                        uuidList.forEach(async (uuid, index) => {
                            await Editor.Message.request('scene', 'create-component', {
                                uuid,
                                component: clipboardComponentInfo.type,
                            });
